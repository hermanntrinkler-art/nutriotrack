...
function BadgeManager() {
  ...
  const handleUpload = async (badgeId: string, file: File) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('badge-images').upload(path, file, { upsert: true });
      if (uploadError) throw uploadError;
      const { data: urlData } = supabase.storage.from('badge-images').getPublicUrl(path);
      const imageUrl = urlData.publicUrl + '?t=' + Date.now();
      const { error: dbError } = await supabase.from('badge_images').upsert({ badge_id: badgeId, image_url: imageUrl }, { onConflict: 'badge_id' });
  ...
  const handleSaveShareText = async (badgeId: string) => {
    try {
      ...
      if (existingImage) {
        await supabase.from('badge_images').update({ share_text: textDraft || null }).eq('badge_id', badgeId);
      } else {
        await supabase.from('badge_images').upsert({ badge_id: badgeId, image_url: '', share_text: textDraft || null }, { onConflict: 'badge_id' });
      }
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground mb-4">Klicke auf ein Badge, um ein eigenes Bild hochzuladen. Bearbeite den Share-Text, der beim Teilen auf Social Media erscheint.</p>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {BADGE_DEFINITIONS.map(badge => {
          ...
          return (
            <div key={badge.id} className="relative border border-border rounded-xl p-3 flex flex-col items-center gap-2 bg-card">
              ...
              {isEditingThis ? (
                <div className="w-full space-y-1">
                  <textarea value={textDraft} onChange={e => setTextDraft(e.target.value)} className="w-full text-[10px] p-1.5 rounded-md border border-border bg-background resize-none" rows={2} placeholder="Share-Text..." />
                  <div className="flex gap-1">
                    <button onClick={() => handleSaveShareText(badge.id)} className="flex-1 text-[9px] font-bold px-2 py-1 rounded-md bg-primary/10 text-primary hover:bg-primary/20">Speichern</button>
                    <button onClick={() => setEditingText(null)} className="text-[9px] font-bold px-2 py-1 rounded-md bg-muted text-muted-foreground hover:bg-muted/80">✕</button>
                  </div>
                </div>
              ) : (
                <button onClick={() => { setEditingText(badge.id); setTextDraft(customText || ''); }} className="text-[9px] font-bold text-muted-foreground hover:text-primary transition-colors">✏️ Share-Text</button>
              )}
              <div className="flex gap-1.5">
                <label className="cursor-pointer">
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleUpload(badge.id, file); e.target.value = ''; }} disabled={isUploading} />
                ...
                {customUrl && (
                  <button onClick={() => handleDelete(badge.id)} className="flex items-center gap-1 px-2 py-1 rounded-md bg-destructive/10 text-destructive text-[10px] font-bold hover:bg-destructive/20 transition-colors">
...
function UserAnalytics() {
  ...
  const loadUsers = async () => {
    const { data: profiles } = await supabase.from('profiles').select('user_id, name, email, created_at, subscription_status').order('created_at', { ascending: false });
    if (!profiles) { setLoading(false); return; }
    const { data: mealData } = await supabase.from('meal_entries').select('user_id, entry_date');
    const mealCountMap: Record<string, { count: number; lastDate: string | null }> = {};
    if (mealData) {
      for (const entry of mealData) {
        if (!mealCountMap[entry.user_id]) mealCountMap[entry.user_id] = { count: 0, lastDate: null };
        mealCountMap[entry.user_id].count++;
        const d = entry.entry_date;
        if (!mealCountMap[entry.user_id].lastDate || d > mealCountMap[entry.user_id].lastDate!) mealCountMap[entry.user_id].lastDate = d;
      }
    }
    setUsers(profiles.map(p => ({
      user_id: p.user_id, name: p.name, email: p.email, created_at: p.created_at,
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="nutri-card text-center">
          <p className="text-2xl font-black text-primary">{users.filter(u => u.subscription_status === 'active').length}</p>
          <p className="text-[10px] text-muted-foreground font-medium">Pro Abos</p>
        </div>
        <div className="nutri-card text-center">
          <p className="text-2xl font-black text-primary">{users.reduce((sum, u) => sum + u.meal_count, 0)}</p>
          <p className="text-[10px] text-muted-foreground font-medium">Mahlzeiten</p>
        </div>
      </div>
      <div className="space-y-2">
        <h3 className="font-semibold text-sm flex items-center gap-2"><Users className="h-4 w-4" /> Alle Nutzer ({users.length})</h3>
        {users.map(u => (
          <div key={u.user_id} className="border border-border rounded-xl p-3 bg-card">
            <div className="flex items-start justify-between">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${u.subscription_status === 'active' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
            ...
            <div className="flex gap-4 mt-2 text-[11px] text-muted-foreground">
              ...
              {u.last_meal_date && <span>🕐 Letztes Meal: {new Date(u.last_meal_date).toLocaleDateString('de-DE')}</span>}
            </div>
          </div>
        ))}
      </div>
...
function FoodReports() {
  ...
  const loadReports = async () => {
    const { data } = await supabase.from('food_reports').select('*').order('created_at', { ascending: false });
    if (data) setReports(data as unknown as FoodReport[]);
    setLoading(false);
  };

  const updateStatus = async (id: string, status: string) => {
    const { error } = await supabase.from('food_reports').update({ status, resolved_at: status !== 'open' ? new Date().toISOString() : null } as any).eq('id', id);
    if (error) { toast.error('Fehler beim Aktualisieren'); }
    else { toast.success(`Status → ${status}`); setReports(prev => prev.map(r => r.id === id ? { ...r, status, resolved_at: status !== 'open' ? new Date().toISOString() : null } : r)); }
  };

  const deleteReport = async (id: string) => {
    const { error } = await supabase.from('food_reports').delete().eq('id', id);
    if (error) { toast.error('Fehler beim Löschen'); }
    else { toast.success('Meldung gelöscht'); setReports(prev => prev.filter(r => r.id !== id)); }
  };

  const updateReport = async (id: string, updates: Partial<FoodReport>) => {
    const { error } = await supabase.from('food_reports').update(updates as any).eq('id', id);
    if (error) { toast.error('Fehler beim Speichern'); return false; }
  ...
  return (
    <div className="space-y-4">
      ...
      {reports.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">Keine Meldungen vorhanden</p>}
      {openReports.length > 0 && (
        <div className="space-y-2">
          <h3 className="font-semibold text-sm flex items-center gap-2"><Flag className="h-4 w-4 text-destructive" /> Offene Meldungen ({openReports.length})</h3>
      ...
      {resolvedReports.length > 0 && (
        <div className="space-y-2">
          <h3 className="font-semibold text-sm flex items-center gap-2 text-muted-foreground"><CheckCircle className="h-4 w-4" /> Erledigte Meldungen ({resolvedReports.length})</h3>
...
function ReportCard({ report, onUpdateStatus, onDelete, onUpdate }: {
  ...
  const handleSaveProduct = async () => {
    ...
    if (error) { toast.error('Fehler beim Speichern der Produktdaten'); }
    else { toast.success('Produktdaten aktualisiert!'); setProductData({ ...editProduct }); setEditProduct(null); }
  };
  ...
  const EditField = ({ label, value, onChange, unit, type = 'number' }: { label: string; value: string | number; onChange: (v: string) => void; unit: string; type?: string }) => (
    <div className="flex items-center gap-2 text-[11px]">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} className="flex-1 p-1.5 rounded-md border border-border bg-background text-xs text-right" step={type === 'number' ? '0.1' : undefined} />
  ...
  return (
    <div className={`border rounded-xl bg-card transition-colors ${isOpen ? 'border-destructive/30' : 'border-border opacity-70'}`}>
      <button onClick={() => { setExpanded(!expanded); setConfirmDelete(false); }} className="w-full p-3 flex items-center justify-between gap-2 text-left">
                    ...
                    <>
                      <div className="rounded-lg bg-muted/50 p-3 space-y-1">
                        <div className="flex items-center justify-between mb-1.5">
                          <p className="text-[10px] font-bold text-muted-foreground uppercase">Nährwerte pro {productData.quantity} {productData.unit}</p>
                          {isCommunity && (
                            <button onClick={() => setEditProduct({ ...productData })} className="flex items-center gap-1 text-[10px] font-bold text-primary hover:text-primary/80 transition-colors">
                      ...
                      {!isCommunity && <p className="text-[10px] text-muted-foreground italic">ℹ️ Nur Community-Produkte können bearbeitet werden</p>}
                    </>
                  )}
